...
- **Lookahead pool OOM on complex meshes**: Octocat with 100 iterations hits pool OOM when n_cutting grows large (many parts × width × width leaf items exhaust the pool). 49160 with 50 iterations converges to ~325 parts with ~23 unsplittable degenerate fragments remaining above threshold.
